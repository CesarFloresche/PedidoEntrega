...
app.config['SQLALCHEMY_DATABASE_URI'] = \
    '{SGBD}://{usuario}:{clave}@{servidor}/{database}'.format(
        ...
        usuario = 'GabrielCes',
        clave = 'LAGABONETA',
        servidor = 'GabrielCes.mysql.pythonanywhere-services.com',
        database = 'GabrielCes$PedidoEntrega'
    )
...
def login():
    if request.method == 'POST':
        ...
        # Esto evita el error "unhashable type: bytearray".
        ...
        if registro_usuario and registro_usuario.clave == password:
            # Aseguramos que lo que guardamos en sesión sea string y no bytes
